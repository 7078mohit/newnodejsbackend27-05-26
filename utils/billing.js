...
const getRateByType = (astrologer, serviceType) => {
  ...
  const chatRate = Number(astrologer?.chatFee || base);
  const voiceRate = Number(astrologer?.callFee || base);
  const videoRate = Number(astrologer?.videoFee || voiceRate);
...
const fetchUserAndAstrologer = async ({ userId, astrologerId }) => {
  ...
  // Support lookup by _id or firebaseUid
  const isObjectId = mongoose.Types.ObjectId.isValid(String(userId || ''));
  const userQuery = isObjectId ? { _id: userId } : { firebaseUid: userId };

  const [user, astrologer] = await Promise.all([
    UserDetail.findOne(userQuery),
...
export const canAffordSessionStart = async ({ userId, astrologerId, serviceType }) => {
...
export const chargeUserUsage = async ({
  userId,
  astrologerId,
  serviceType,
  minutes,
