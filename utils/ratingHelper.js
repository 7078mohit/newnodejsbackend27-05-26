...
export const getRatingSummaryForAstros = async (astroIds) => {
  ...
  const data = await RatingReview.aggregate([
    ...
    {
      $group: {
        _id: { astrologer: '$astrologer', rating: '$rating' },
        count: { $sum: 1 },
      },
    },
    {
      $group: {
        _id: '$_id.astrologer',
        stars: { $push: { rating: '$_id.rating', count: '$count' } },
        totalReviews: { $sum: '$count' },
        weightedSum: { $sum: { $multiply: ['$_id.rating', '$count'] } },
      },
    },
    {
      $project: {
